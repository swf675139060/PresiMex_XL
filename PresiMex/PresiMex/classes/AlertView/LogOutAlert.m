...
@interface LogOutAlert()<UITableViewDelegate,UITableViewDataSource>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        WFLabelCell * cell = [WFLabelCell cellWithTableView:tableView];
        cell.label.text = @"Confirmar la salida?";
        cell.label.textAlignment = NSTextAlignmentCenter;
        cell.label.textColor = [UIColor jk_colorWithHexString:@"#1B1200"];
        ...
        [cell upBGFrameWithInsets:UIEdgeInsetsMake(40, 25, 25, 25) height:50];
        [cell.leftBtn setTitle:@"Cancelación" forState:UIControlStateNormal];
        ...
        [cell.rightBtn addLinearGradientwithSize:CGSizeMake((WF_ScreenWidth - 119)/2, 50) maskedCorners:kCALayerMinXMinYCorner | kCALayerMaxXMinYCorner | kCALayerMinXMaxYCorner | kCALayerMaxXMaxYCorner cornerRadius:13];
        
        [cell.leftBtn mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(@(0));
            make.top.equalTo(@(0));
            make.bottom.equalTo(@(0));
            make.width.equalTo(@((WF_ScreenWidth - 119)/2));
