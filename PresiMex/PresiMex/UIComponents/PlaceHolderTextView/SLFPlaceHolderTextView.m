...
CGFloat const kFSTextViewPlaceholderHorizontalMargin = 8.0; ///< placeholder水平方向边距

@interface SLFPlaceHolderTextView ()

@property (nonatomic, copy) SLFTextViewHandler changeHandler; ///< 文本改变Block
@property (nonatomic, copy) SLFTextViewHandler maxHandler; ///< 达到最大限制字符数Block
@property (nonatomic, strong) UILabel *placeholderLabel; ///< placeholderLabel

@end

@implementation SLFPlaceHolderTextView
...
- (void)initialize
{
    ...
    self.placeholderLabel.slf_lineSpace = 5;
...
- (void)addTextDidChangeHandler:(SLFTextViewHandler)changeHandler
{
    _changeHandler = [changeHandler copy];
}

- (void)addTextLengthDidMaxHandler:(SLFTextViewHandler)maxHandler
{
